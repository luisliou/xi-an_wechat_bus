...
// --------------------- 消息返回 -------------------------
// 返回空消息
Weixin.prototype.sendEmptyMsg = function(msg) {
	var time = Math.round(new Date().getTime() / 1000);
	
	var curResKey = '';
	curResKey = msg.fromUserName + msg.createTime;
	console.log('To be sent, key:' + curResKey + '\nresArray len:' + resArray.length);
	var curRes = resArray[curResKey];
	curRes.type('xml'); 
	curRes.send('');
	delete resArray[curResKey];

	return this;
}
// 返回文字信息
Weixin.prototype.sendTextMsg = function(msg, sentTime) {
	var time = Math.round(new Date().getTime() / 1000);
	...
	var curResKey = '';
	curResKey = msg.toUserName + sentTime;
	console.log('To be sent, key:' + curResKey + '\nresArray len:' + resArray.length);
	var curRes = resArray[curResKey];
	curRes.type('xml'); 
	curRes.send(output);
...
// 发送信息
Weixin.prototype.sendMsg = function(msg, sentTime) {
	switch(msg.msgType) {
		
		case 'text' : 
			this.sendTextMsg(msg, sentTime);
			break;
...
Weixin.prototype.loop = function(req, res) {	
    ...
    req.on('end', function() {
		...
		var curResKey = '';
		curResKey = self.data.FromUserName[0] + self.data.CreateTime;
		console.log('res key:' + curResKey);
		resArray[curResKey] = res;
		self.parse();
    });
}
